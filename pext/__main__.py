    ...
    def _process_tab_action(self, tab: Dict, active_tab: int) -> None:
        ...
        if action[0] == Action.critical_error:
            ...
            if Settings.get("output_mode") == OutputMode.AutoType:
                ...
                Logger.log(
                    tab["metadata"]["name"],
                    Translation.get("data_queued_for_clipboard"),
                )
            ...
            tab["vm"].context.setContextProperty(
                "resultListModelTree",
                [part["value"] for part in tab["vm"].selection])
...
class ProfileManager:
    ...
    def rename_profile(self, old_name: str, new_name: str) -> bool:
        ...
        try:
            os.rename(
                os.path.join(self.profile_dir, old_name),
                os.path.join(self.profile_dir, new_name),
            )
        except OSError:
            return False
...
class ModuleManager:
    ...
    def _pip_install(self, identifier: str) -> Optional[str]:
        ...
        pip_command = [sys.executable, "-m", "pip", "install", "--isolated"]
    ...
    def load(self, window: "Window", module: Dict[str, Any]) -> bool:
        ...
        for name, value in getmembers(ModuleBase, isfunction):
            required_param_lengths[name] = (
                len(signature(value).parameters) - 1
            )  # Python is inconsistent with self
    ...
    def reload(self, window: "Window", tab_id: int,
               in_between_callback=None) -> bool:
    ...
    def update(self, identifier: str, verbose=False) -> bool:
        ...
        if verbose:
            Logger.log(None,
                       Translation.get("updating_dependencies").format(name))
...
class ViewModel:
    ...
    def go_up(self, to_base=False) -> None:
        ...
        if len(self.selection) > 0:
            ...
            self.context.setContextProperty(
                "resultListModelTree",
                [part["value"] for part in self.selection])
    ...
    def select(self, command_args="", force_args=False,
        ...
        self.context.setContextProperty("contextMenuEnabled", False)
        self.context.setContextProperty(
    ...
    def show_context(self) -> None:
        ...
        self.context.setContextProperty("contextMenuEnabled", True)
    ...
    def update_context_info_panel(self, request_update=True) -> None:
        ...
        try:
            if current_entry["type"] == SelectionType.entry:
                QQmlProperty.write(
                    self.context_info_panel,
                    "text",
                    self.extra_info_entries[current_entry["value"]],
                )
            else:
                QQmlProperty.write(
                    self.context_info_panel,
                    "text",
                    self.extra_info_commands[current_entry["value"]],
                )
        except KeyError:
            QQmlProperty.write(self.context_info_panel, "text", "")
...
class Window:
    ...
    def __init__(self,
        ...
        QQmlProperty.write(
            self.menu_enable_global_hotkey_shortcut,
            "checked",
        ...
        QQmlProperty.write(
            self.menu_enable_object_update_check_shortcut,
            "checked",
            Settings.get("object_update_check"),
        )
        QQmlProperty.write(
            self.menu_enable_object_update_install_shortcut,
            "checked",
            Settings.get("object_update_check")
            and Settings.get("object_update_install"),
        )
        ...
        if platform.system() == "Darwin" and Settings.get("background"):
            ...
            if Settings.get("update_check") is None and USE_INTERNAL_UPDATER:
                ...
                self.add_actionable(
                    ...
                    Translation.get("actionable_update_check_enabled"),
                )
        ...
        if not self.app.platformName() in ["webgl", "vnc"]:
            ...
            try:
                ...
                self.window.setGeometry(
                    (screen_size.width() - 800) / 2,
                    (screen_size.height() - 600) / 2,
                    800,
                    600,
                )
        ...
        if parts[1] == "update-module-in-use":
            ...
            for tab_id, tab in enumerate(self.tab_bindings):
                if tab["metadata"]["id"] == parts[2]:
                    reload_chain = partial(self.module_manager.reload, self,
                                           tab_id, reload_chain)
    ...
    def _update_themes_info_qml(self) -> None:
        ...
        self.context.setContextProperty("themes", themes)
    ...
    def load(self, identifier: str) -> QPalette:
        ...
        for colour_group in config.sections():
            for colour_role in config[colour_group]:
                ...
                try:
                    palette.setColor(
                        palette_mappings["colour_groups"][colour_group],
...
def _init_persist(profile: str, background: bool) -> None:
    ...
    if lock:
        ...
        if not background:
            ...
            print(
                "Pext is already running, but --background was given. Doing nothing..."
            )
...
def _parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--background",
        action="store_true",
        help="do not open the user interface this invocation.",
    )
    parser.add_argument(
        "--output",
        choices=[
    ...
    if args.install_module:
        for metadata_url in args.install_module:
            try:
                ...
                try:
                    translated_metadata_url = re.sub(
                        r"\.json$",
                        "_{}.json".format(
                            LocaleManager.find_best_locale(
                                Settings.get("locale")).name()),
...
def main() -> None:
    ...
    observer.schedule(
        event_handler,
        os.path.join(ConfigRetriever.get_path(), "modules"),
        recursive=True,
    )
    observer.start()
