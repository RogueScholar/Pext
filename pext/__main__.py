...
class ViewModel:
    ...
    def update_context_info_panel(self, request_update=True) -> None:
        ...
        if (
            current_entry["value"] == self.extra_info_last_entry
...
class Window:
    ...
    def __init__(
        self, app: QApplication, locale_manager: LocaleManager, parent=None
    ) -> None:
        ...
        menu_load_module_shortcut = self.window.findChild(QObject, "menuLoadModule")
            ...
            for tab_id, tab in enumerate(self.tab_bindings):
                if tab["metadata"]["id"] == parts[2]:
                    reload_chain = partial(
                        self.module_manager.reload, self, tab_id, reload_chain
                    )
